...
class MongoManager:
    ...
    """def get(self, **kwargs):
        ...
        projection = kwargs.get("filter", {})
        skip = kwargs.get("skip", 0)
        limit = kwargs.get("limit", 0)
        sort_criteria = kwargs.get("sort_criteria", [])

        try:
            # Using find() method for multiple documents retrieval
            results_cursor = self.collection.find(query, projection).sort(sort_criteria).skip(skip).limit(limit)
...
"""
        ...
        skip = kwargs.get("skip", 0)
        limit = kwargs.get("limit", 0)
        sort_criteria = kwargs.get("sort_criteria", [("__id,1")])
        ...
        try:
            ...
            """if results is not None:  # Check if the result is not None
                return {
                    "success": True,
                    "result_size": 1,
                    "data": results  # Directly return the result as it's a single document, not a list
                    }
